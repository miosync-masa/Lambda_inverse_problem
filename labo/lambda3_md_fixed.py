...
def evaluate_detection_performance(result: MDLambda3Result,
    ...
    # 現在は同じ境界が複数イベントに使われるため、一時的にコメントアウト推奨
    print("\n\n📊 Level 2: Boundary Timing Accuracy")
    print("-" * 40)
...
def demo_md_50k_analysis():
    ...
    try:
        ...
        try:
            trajectory = np.load('/content/lysozyme_50k_trajectory.npy').astype(np.float64)
            backbone_indices = np.load('/content/lysozyme_50k_backbone_indices.npy')
        except FileNotFoundError:
            # Fallback: use first 50k frames from 100k dataset
            print("   50k files not found, using first 50k frames from 100k dataset...")
            trajectory = np.load('/content/lysozyme_100k_final_challenge.npy').astype(np.float64)[:50000]
            backbone_indices = np.load('/content/lysozyme_100k_backbone_indices.npy')
...
def demo_md_analysis_100k():
    ...
    try:
        trajectory = np.load('/content/lysozyme_100k_final_challenge.npy').astype(np.float64)
        backbone_indices = np.load('/content/lysozyme_100k_backbone_indices.npy')
        print(f"Loaded trajectory: {trajectory.shape}")
        print(f"Loaded backbone indices: {len(backbone_indices)} atoms")
    except FileNotFoundError:
