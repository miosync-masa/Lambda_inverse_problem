...
def extract_md_features(trajectory: np.ndarray, 
    ...
    if config.use_contacts:
        contact_maps = np.zeros((n_frames, n_atoms, n_atoms))
        contact_changes = np.zeros(n_frames)
        
        for i in range(n_frames):
            contact_maps[i] = calculate_contact_map(trajectory[i], CONTACT_CUTOFF)
            if i > 0:
                contact_changes[i] = np.sum(np.abs(contact_maps[i] - contact_maps[i-1]))
        
        # Flatten contact map for each frame
        features['contact_vector'] = contact_maps.reshape(n_frames, -1)
...
def demo_md_analysis():
    ...
    try:
        trajectory = np.load('lysozyme_100k_trajectory.npy')
    ...
    # Find top anomalies
    top_10_frames = np.argsort(anomaly_scores)[-10:][::-1]
    print(f"\nTop 10 anomalous frames: {top_10_frames}")
    ...
    print(f"    Frames 400-450 (unfolding): mean score = {np.mean(anomaly_scores[400:450]):.3f}")
    print(f"    Frames 600-650 (misfolding): mean score = {np.mean(anomaly_scores[600:650]):.3f}")
    print(f"    Frames 0-100 (baseline): mean score = {np.mean(anomaly_scores[0:100]):.3f}")
